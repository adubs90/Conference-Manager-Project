...

{
	...
	private JButton createAccountButton = new JButton("Creat an Account");
	...
	public LoginPanel()
	{
		...
		final JTextField loginInfo = new JTextField("DiazP@weyerhaeuser.com", 36);
		...
		public void actionPerformed(ActionEvent e) {
			...
			for (int i =0; i < theData.getUsers().size(); i++) {
				if (theData.getUsers().get(i).getEmail().equals(theUser)) {
					...
					User foundUser = theData.getUsers().get(i);
	...
	myLoginButton.addActionListener(new ActionListener()
	{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(foundUser == null)
			{
				JOptionPane.showMessageDialog(null, theUser + " doesn't exist! Create Account?");			
